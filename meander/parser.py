...
async def parse_server(reader, document):
    ...
    if toks[2] != "HTTP/1.1":
        raise HTTPException(400, "Bad Request", f"unsupported HTTP protocol: {toks[2]}")
...
async def parse_client(reader, document):
    ...
    try:
        ...
        raise HTTPException(
            400, "Bad Request", f"invalid status code: {toks[1]}"
        ) from exc
...
async def parse_headers_and_body(reader, document):  # pylint: disable=too-many-branches
    ...
    # --- content type
    if (
        "content-type" in document.http_headers
        and document.http_headers.get("content-type") == ""
    ):
    ...
    if encoding:
        if encoding != "gzip":
            raise HTTPException(400, "Bad Request", "unsupported content encoding")
    ...
    if document.http_content and document.http_encoding == "gzip":
        try:
            ...
            raise HTTPException(400, "Bad Request", "malformed gzip data") from exc
...
async def parse_http_content(reader, document):
    ...
    try:
        ...
        raise HTTPException(400, "Bad Request", "invalid content-length") from exc
    ...
    if document.http_content_type == "application/json":
        ...
        if document.http_content:
            query = urlparse.parse_qs(
                document.http_content.decode("ascii"), keep_blank_values=True
            )
            document.content = {n: v[0] if len(v) == 1 else v for n, v in query.items()}
