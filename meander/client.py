...
class Client:  # pylint: disable=too-many-instance-attributes
    ...
    async def open(self, timeout=60, active_timeout=5, max_read_size=5000):
        ...
        http_reader = HTTPReader(
            reader, is_server=False, timeout=timeout,
            active_timeout=active_timeout, max_read_size=max_read_size)
    ...
    def write(self,  # pylint: disable=too-many-arguments
              method="GET", path="/", query="", headers=None, content="",
              host=None, content_type=None, charset="utf-8", close=False,
              compress=False):
        """send an HTTP document on the connection"""
        self.payload = ClientResponse(
            method=method, path=path, query=query, headers=headers,
            content=content, host=host, content_type=content_type,
            charset=charset, close=close, compress=compress)
        self.writer.write(self.payload.serial())

    async def execute(self, method, path="/", query="", content="", host=None,
...
async def call(  # pylint: disable=too-many-arguments
        url, method="GET", content="", headers=None, content_type=None,
        charset="utf-8", compress=False, bearer=None, timeout=60,
        active_timeout=5, max_read_size=5000, verbose=False):
    ...
    client = Client(url)
    await client.open(timeout=timeout, active_timeout=active_timeout,
                      max_read_size=max_read_size)
    client.write(
        method=method, path=client.url.path, query=client.url.query,
        headers=headers, content=content, host=client.url.host,
        content_type=content_type, charset=charset, close=True,
        compress=compress)
    if verbose:
